...
// Default initial CMS configuration for Kiwik.1
  ...
  pages: [
    {
      ...
      sections: [
        ...
        },
        {
          id: "sec-dashboard",
          type: "features",
          title: "Desktop Workspace Dashboard",
          content: "Real-time telemetry and project execution system.",
          visible: true,
          order: 2
        }
      ],
  ...
  persist(
    (set, get) => ({
      ...
      updateSettings: (partialSettings) => {
        set((state) => {
          const updatedSettings = { ...state.cms.settings, ...partialSettings };
          return { cms: { ...state.cms, settings: updatedSettings } };
        });
      ...
      updateHero: (partialHero) => {
        set((state) => {
          const updatedHero = { ...state.cms.hero, ...partialHero };
          return { cms: { ...state.cms, hero: updatedHero } };
        });
      ...
      updateHeroRotatingWords: (words) => {
        set((state) => ({
          cms: {
            ...state.cms,
            hero: { ...state.cms.hero, rotatingWords: words }
          }
        }));
      ...
      updateHeroMetric: (id, partialMetric) => {
        set((state) => {
          const updatedMetrics = state.cms.hero.metrics.map((m) =>
            m.id === id ? { ...m, ...partialMetric } : m
          );
          return {
            cms: {
              ...state.cms,
              hero: { ...state.cms.hero, metrics: updatedMetrics }
            }
          };
        });
      ...
      updateNavigation: (partialNav) => {
        set((state) => {
          const updatedNav = { ...state.cms.navigation, ...partialNav };
          return { cms: { ...state.cms, navigation: updatedNav } };
        });
        get().addAuditLog("UPDATE_NAVIGATION", "Navigation", "Updated navbar items or branding");
      },

      updateFooter: (partialFooter) => {
        set((state) => {
          const updatedFooter = { ...state.cms.footer, ...partialFooter };
          return { cms: { ...state.cms, footer: updatedFooter } };
        });
      ...
      updateTheme: (partialTheme) => {
        set((state) => {
          const updatedTheme = { ...state.cms.theme, ...partialTheme };
          return { cms: { ...state.cms, theme: updatedTheme } };
        });
      ...
      updateSEO: (partialSEO) => {
        set((state) => {
          const updatedSEO = { ...state.cms.seo, ...partialSEO };
          return { cms: { ...state.cms, seo: updatedSEO } };
        });
      ...
      deleteMediaItem: (id) => {
        set((state) => ({
          cms: {
            ...state.cms,
            media: state.cms.media.filter((m) => m.id !== id)
          }
        }));
      ...
      createSnapshot: (name, note = "") => {
        ...
        set((state) => ({
          cms: {
            ...state.cms,
            snapshots: [snapshot, ...state.cms.snapshots]
          }
        }));
      ...
      rollbackSnapshot: (snapshotId) => {
        ...
        try {
          ...
          set((state) => ({
            cms: {
              ...restoredCMS,
              snapshots: state.cms.snapshots, // Preserve snapshot history list
              auditLogs: [
                {
                  id: `log-${Date.now()}`,
                  timestamp: new Date().toISOString(),
      ...
      deleteSnapshot: (snapshotId) => {
        set((state) => ({
          cms: {
            ...state.cms,
            snapshots: state.cms.snapshots.filter((s) => s.id !== snapshotId)
          }
        }));
      },
      ...
      addAuditLog: (action, section, details) => {
        ...
        set((state) => ({
          cms: {
            ...state.cms,
            auditLogs: [log, ...state.cms.auditLogs.slice(0, 100)] // Keep last 100 logs
          }
        }));
      },

      exportJSONBackup: () => {
        return JSON.stringify(get().cms, null, 2);
      },
