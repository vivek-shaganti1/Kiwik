...
// Curated art-directed images matching the reference screenshots (optimized for zero-lag 60FPS)
const RIBBON_IMAGES = [
  {
    id: "img-1",
...
export function ImageRibbon() {
  ...
  // Mouse Parallax Values with optimized spring config
  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-[1500px] mx-auto py-4 sm:py-8 flex items-center justify-center overflow-visible select-none transform-gpu">
      ...
      <motion.div
        ...
        className="relative z-10 flex items-center justify-center gap-1.5 sm:gap-3 px-4 overflow-visible transform-gpu will-change-transform"
      >
        {RIBBON_IMAGES.map((img, idx) => (
          <motion.div
            ...
            className={cn(
              "relative rounded-xl sm:rounded-2xl overflow-hidden shadow-2xl border border-white/20 transition-all cursor-pointer group shrink-0 transform-gpu will-change-transform",
              img.aspect
            )}
          >
